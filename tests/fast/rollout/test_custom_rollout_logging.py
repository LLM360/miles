...
def test_eval_hook_can_add_metrics_when_rollout_output_has_none(monkeypatch):
    ...
    result = rollout.log_eval_rollout_data(3, args, {"heldout": {"rewards": [1.0]}}, extra_metrics=None)
...
def test_train_hook_adds_metrics_to_existing_tracking_call(monkeypatch):
    ...
    monkeypatch.setattr(rollout, "_compute_perf_metrics_from_samples", lambda _args, _samples, _time: {})
    monkeypatch.setattr(
        rollout.tracking,
        "log",
        lambda _args, metrics, step_key: captured.update(metrics),
    ...
    rollout.log_rollout_data(3, args, [], {}, 1.0)
