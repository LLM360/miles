...
@pytest.mark.parametrize("permits", [0, 1])
async def test_external_cancel_restores_a_simultaneously_acquired_permit(permits):
    state = SimpleNamespace(
        generate_fn_semaphore=asyncio.Semaphore(permits), abort_event=asyncio.Event(), aborted=False
    )
    task = asyncio.create_task(_acquire_or_abort(state))
    await asyncio.sleep(0)
    await asyncio.sleep(0)
    task.cancel()
    with pytest.raises(asyncio.CancelledError):
        await task
    assert state.generate_fn_semaphore._value == permits
