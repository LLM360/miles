...
def test_over_sampling_rounds(rollout_env, expected_rounds):
    ...
    assert (
        metrics["rollout/groups_filter_rejected"] + metrics["rollout/pending_groups_at_abort"]
    ) == expected_requests - expected_rounds
    ...
    assert (
        metrics["rollout/pending_trajectories_at_abort"]
        == metrics["rollout/pending_groups_at_abort"] * env.args.n_samples_per_prompt
    )
    ...
    [
        pytest.param(
            integration_env_config(
                ...
                data_rows=[{"input": f"accepted-{index}", "label": "valid"} for index in range(10)],
            ),
...
def test_initial_oversampling_submits_one_extra_wave(rollout_env):
    ...
    assert out.metrics["rollout/groups_unused_completed"] + out.metrics["rollout/pending_groups_at_abort"] == 2
...
def _install_controlled_completion(monkeypatch, *, hold_accepted=False):
    ...
    def submit(_state, groups, sample_done_callback=None):
        nonlocal submitted
        tasks = []
        for group in groups:
            first = group[0][0] if isinstance(group[0], list) else group[0]
            ...
            task = asyncio.create_task(complete(group, gate), name=f"controlled-{next(task_ids)}")
            tasks.append(task)
    ...
    async def wait_released(pendings, return_when):
        del return_when
        ready = {task for task in pendings if task_gates[task] is None or task_gates[task].is_set()}
        assert ready
    ...
    [
        pytest.param(
            integration_env_config(
                ...
                data_rows=[{"input": f"accepted-{index}", "label": "valid"} for index in range(3)],
            ),
...
def test_multiple_kept_completions_are_accounted_as_unused(rollout_env, monkeypatch):
    _install_controlled_completion(monkeypatch)
