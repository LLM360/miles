...
def test_preserve_outcomes_keep_engine_status(exit_status: str, effective_response_length: int) -> None:
    ...
    assert (
        resolve_sample_status(Sample.Status.COMPLETED, effective_response_length, outcome) is Sample.Status.COMPLETED
    )
...
@pytest.mark.parametrize("effective_response_length", [0, 7])
def test_other_failures_always_abort_and_reject(exit_status: str, effective_response_length: int) -> None:
    outcome = classify_exit_status(exit_status)
