...
def test_invalid_outcome_filter_rejects_unusable_sample_statuses(status: Sample.Status, reason: str) -> None:
...
def test_composed_filter_rejects_zero_std_after_outcome_checks(reward: int, expected_reason: str) -> None:
    output = check_no_invalid_outcomes_then_nonzero_std(ARGS, [_sample(reward), _sample(reward)])
...
def test_nontrainable_sample_filter_is_defensive(exit_status: str, status: Sample.Status, expected_mask: bool) -> None:
