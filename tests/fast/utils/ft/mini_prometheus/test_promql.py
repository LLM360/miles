...
class TestParsePromQL:
    def test_simple_metric_name(self) -> None:
        expr = parse_promql("miles_ft_gpu_available")
        assert isinstance(expr, MetricSelector)
        assert expr.name == "miles_ft_gpu_available"
        assert expr.matchers == []

    def test_metric_with_label_filter(self) -> None:
        expr = parse_promql('miles_ft_xid_code_recent{xid="48"}')
        assert isinstance(expr, MetricSelector)
        assert expr.name == "miles_ft_xid_code_recent"
    ...
    def test_compare_eq(self) -> None:
        expr = parse_promql("miles_ft_gpu_available == 0")
        assert isinstance(expr, CompareExpr)
        assert expr.selector.name == "miles_ft_gpu_available"
