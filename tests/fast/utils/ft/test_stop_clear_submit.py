...
class TestStopClearSubmit:
    ...
    @pytest.mark.anyio
    async def test_stop_failure_still_submits(self) -> None:
        training_job = FakeTrainingJob()
        mini_wandb = MiniWandb()
    ...
    @pytest.mark.anyio
    async def test_mini_wandb_cleared_even_on_stop_failure(self) -> None:
        training_job = FakeTrainingJob()
        mini_wandb = MiniWandb()
        ...
        await stop_clear_submit(training_job, mini_wandb)
