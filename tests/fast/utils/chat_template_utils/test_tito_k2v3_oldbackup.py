...
K2V3_MODEL_PATH = os.environ.get(
    "TITO_TEST_MODEL_PATH_K2V3_OLDBACKUP",
...
_K2V3_REASONING_EFFORT = os.environ.get("TITO_TEST_REASONING_EFFORT_K2V3_OLDBACKUP", "high")
...
# environments — e.g. ``TITO_TEST_TOOL_PARSER_K2V3_OLDBACKUP=qwen25``. If the
...
# turning green.
_K2V3_TOOL_PARSER = os.environ.get("TITO_TEST_TOOL_PARSER_K2V3_OLDBACKUP", "hermes")
_K2V3_REASONING_PARSER = os.environ.get("TITO_TEST_REASONING_PARSER_K2V3_OLDBACKUP", "deepseek-r1")
...
def tokenizer() -> AutoTokenizer:
    ...
    return load_tokenizer(
        K2V3_MODEL_PATH,
        chat_template_path=resolve_fixed_chat_template(TITOTokenizerType.K2V3_OLDBACKUP)[0],
...
def test_legacy_tokenizer_rejects_vocab_without_legacy_marker(tokenizer):
    from miles.utils.chat_template_utils.tito_tokenizer import K2V3OldBackupTITOTokenizer

    class NoLegacyMarker:
        def __getattr__(self, name):
            return getattr(tokenizer, name)

        def convert_tokens_to_ids(self, token):
            return None if token == "<|im_end|>" else tokenizer.convert_tokens_to_ids(token)

    with pytest.raises(ValueError, match="requires <"):
        K2V3OldBackupTITOTokenizer(NoLegacyMarker())
