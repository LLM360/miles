...
async def generate_rollout_async(
    ...
    try:
        while len(data) < target_data_size:
            ...
            # fetching replacement prompts so its health can be judged as a
            # bounded unit.
            if not pendings and not source_exhausted:
    ...
    if stop_training:
        ...
        logger.info(
            "[rollout] terminating training after partial final rollout: accepted=%s/%s submitted_groups=%s",
            len(data),
    ...
    if stop_training:
        logger.warning(
            f"[rollout] consumed prompt budget exhausted: {len(data)}/{args.rollout_batch_size} "
            "groups survived the dynamic filter; partial rollout will not be trained"
        )
    ...
    flat_all = [
        ...
        for sample in (
            group if not isinstance(group[0], list) else [sample for sub_group in group for sample in sub_group]
        )
    ]
    scalar_rewards = [
        reward for sample in flat_all if isinstance(reward := sample.get_reward_value(args), (int, float))
    ]
    if scalar_rewards:
