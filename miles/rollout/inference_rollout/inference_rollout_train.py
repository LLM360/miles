...
from miles.rollout.filter_hub.base_types import MetricGatherer, iter_samples
...
from miles.utils.http_utils import get, router_worker_base_urls
...
async def _signal_harbor(client: httpx.AsyncClient, harbor_url: str, rollout_id: int, timeout: float) -> None:
...
async def _abort_all_engines(client: httpx.AsyncClient, args: Namespace) -> None:
    if parse(sglang_router.__version__) <= parse("0.2.1") or args.use_miles_router:
        ...
        path = "/workers"
    response = await client.get(f"http://{args.sglang_router_ip}:{args.sglang_router_port}{path}")
    response.raise_for_status()
    body = response.json()
    urls = body["urls"] if path == "/list_workers" else [worker["url"] for worker in body["workers"]]
    urls = router_worker_base_urls(urls)
    logger.info(f"[abort] abort_all -> {urls}")
    results = await asyncio.gather(
        *[client.post(f"{url}/abort_request", json={"abort_all": True}) for url in urls],
...
async def abort(state: GenerateState, pendings: set, rollout_id: int) -> list[list[Sample]]:
    ...
    signal_timeout, harbor_timeout = _abort_signal_budgets(deadline - asyncio.get_running_loop().time())
    ...
    async with httpx.AsyncClient(timeout=signal_timeout, limits=limits) as client:
        ...
        if is_agentic:
            ...
            signals.append(call_agent_abort_hook(args))
        signals.append(_abort_all_engines(client, args))
        try:
            ...
            signal_failures = [result for result in signal_results if isinstance(result, BaseException)]
    ...
    def keep_partial_group(group: list[Sample] | None) -> None:
        ...
        for sample in iter_samples(group):
            if (sample.effective_response_length > 0 or sample.response) and "start_rollout_id" not in sample.metadata:
