...
# tokens or context. Real rollouts (verifier reward is meaningful) but the
...
def mask_truncated_and_llm_judge_failed(args, samples: list[Sample]) -> None:
    ...
    """
    for sample in _flatten_samples(samples):
        if sample.status == Sample.Status.TRUNCATED or (sample.metadata or {}).get("llm_judge_failed"):
            sample.remove_sample = True
