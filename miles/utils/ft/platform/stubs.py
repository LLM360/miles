...
class StubNodeManager:
    ...
    async def mark_node_bad(self, node_id: str, reason: str) -> None:
        log.info("stub_mark_node_bad node_id=%s reason=%s", node_id, reason)

    async def unmark_node_bad(self, node_id: str) -> None:
        log.info("stub_unmark_node_bad node_id=%s", node_id)
...
class StubTrainingJob:
    ...
    async def stop_training(self, timeout_seconds: int = 300) -> None:
        log.info("stub_stop_training timeout_seconds=%d", timeout_seconds)

    async def submit_training(self) -> str:
        run_id = uuid4().hex[:8]
        log.info("stub_submit_training run_id=%s", run_id)
        return run_id
