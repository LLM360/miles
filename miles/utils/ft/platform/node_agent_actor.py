...
class _FtNodeAgentActorCls:
    ...
    and registers itself with the FtController.

    Diagnostics are no longer embedded in the node agent — they run in
    temporary DiagnosticActor instances spawned by the orchestrator.
    """
    ...
    def _register_with_controller(self) -> None:
        ...
        if result.ok:
            logger.info(
                "Node agent registered node_id=%s exporter=%s ray_node_id=%s",
                self._node_id, exporter_address, ray_node_id,
            )
