...
def validate_args(args):
    ...
    if args.sglang_router_policy:
        ...
        assert (
            not enable_experimental_rollout_refactor() or args.sglang_router_policy != "consistent_hashing"
        ), "--sglang-router-policy=consistent_hashing is not supported with MILES_EXPERIMENTAL_ROLLOUT_REFACTOR=1 (no client-side session routing in experimental path)"
